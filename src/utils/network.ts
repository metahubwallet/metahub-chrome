...
const networkIcons = import.meta.glob<{ default: string }>(
    '@/assets/images/network_icon/*.png',
    { eager: true }
);

function resolveNetworkIcon(name: string): string {
    const key = Object.keys(networkIcons).find((k) => k.endsWith(`/${name}.png`));
    return key ? networkIcons[key].default : '';
}

export const getNetworkLocalIcon = (chain: string = 'eos', active = true): string => {
    let network = chain.toLowerCase();
    if (network === 'jungle3') network = 'jungle';
    network = network.replace('-test', '');

    // Check if it is a supported network
    const networks = ['bos', 'jungle', 'telos', 'kylin', 'wax', 'lynx', 'meet', 'proton'];
    const isExist = networks.findIndex((item) => item === network);
    let icon = '';
    ...
    return resolveNetworkIcon(icon);
};
