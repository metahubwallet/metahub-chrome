...
// After configuration, refresh the entire extension at chrome://extensions
export const localCache = {
    ...
    async get(key: CacheKey, defaultValue: any = null): Promise<any> {
        const items: Record<string, any> = await chrome.storage.local.get(key);
    ...
    async upgrade() {
        ...
        if (data && data.passwordHash) {
            ...
            const states: Record<string, any> = await chrome.storage.local.get(keys);
            for (const k of keys) {
                let nk = k;
                if (nk == 'selectedRpc') {
                    nk = 'selectedRpcs';
